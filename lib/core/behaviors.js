    ...
    nextMessage: for (const m of update.messages) {
        ...
        const groupChat = group
            ? {
                ...
                getParticipants: () => group.participants.map(p => ({ id: p.phoneNumber, lid: p.id })),
                getAdmins: () => group.participants.filter(p => p.admin).map(p => ({ id: p.phoneNumber, lid: p.id })),
                getMembers: () => group.participants.filter(p => !p.admin).map(p => ({ id: p.phoneNumber, lid: p.id })),
                getLink: async () => {
                    const code = await getSocket()?.groupInviteCode(group.id)
                    return code ? `https://chat.whatsapp.com/${await getSocket()?.groupInviteCode(group.id)}` : null
                }
            } : null
