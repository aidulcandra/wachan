...
class Message {
    constructor (m, context) {
        ...
        this.isPrivate = this.room.endsWith(USER_JID) || this.room.endsWith(USER_LID)
        const senderIds = [ m.key.participant, m.key.participantAlt, m.key.remoteJid, m.key.remoteJidAlt ]
        const senderId = m.key.fromMe 
            ? botInfo.id
            : senderIds.find(i=>i?.endsWith("@s.whatsapp.net"))
        const senderLid = m.key.fromMe 
            ? botInfo.lid
            : senderIds.find(i=>i?.endsWith("@lid"))
        this.sender = {
            id: senderId,
            lid: senderLid,
            isMe: m.key.fromMe,
        ...
        for (const key in realMessage) {
            ...
            switch (key) {
                ...
                case "documentMessage": {
                    this.type = content.gifPlayback ? "gif"
                        : content.ptt ? "vn"
                        : key.slice(0,-7)
                    this.isMedia = true
                    ...
                    this.streamMedia = function () {
                        const socket = getSocket()
                        const stream = downloadMediaMessage(m, "stream", {}, {reuploadRequest: socket.updateMediaMessage})
                        return stream
                    }
                    this.streamMedia = function () {
                        const socket = getSocket()
                    ...
                    if (content.fileName) this.fileName = content.fileName
                    if (content.seconds) this.duration = content.seconds
                    if (content.height) this.height = content.height
                    if (content.width) this.width = content.width
                    this.mimeType = content.mimetype
                    this.fileSize = content.fileLength.toNumber()
                    
                    break
                }
                case "reactionMessage": {
                    this.reaction = { emoji: content.text, key: content.key }
                    break
                }
                case "interactiveMessage": {
                    this.type = "buttons"
                    this.text = content.body.text
                    this.title = content.header.title
                    this.footer = content.footer.text
                    this.buttons = content.nativeFlowMessage.buttons
                        .map(b => {
                ...
                case "contactsArrayMessage": {
                    ...
                    this.contacts = [
                        content.vcard || 
                        content.contacts?.map(c => c.vcard)
                ...
                case "protocolMessage": {
                    if (content.type === 14) { // Edited message
                        this.type = "edit"
                        const [ editType, editContent ] = Object.entries(content.editedMessage)[0]
                        this.edited = {
                            type: editType === "extendedTextMessage" ? "text"
                                : editType.replace(/Message$/, ""),
                            text: editContent.text || editContent.caption || "",
                        }
                    }
                    break
                }
            ...
            const t = content?.text 
                || content?.caption 
                || content
            if (typeof t === "string" && this.type !== "reaction" && this.type !== "edit") this.text = t
        }
    ...
    async edit (newText) {
        return await editMessage(this, newText)
    }
...
async function sendMessage(room, options) {
    ...
    const mediaTypes = [
        ...
        ["audio", {mimetype:"audio/mp4", ptt:true}, "vn"],
        ["sticker"],
        ["document"]
    ]
        ...
        for (const key in options) {
            ...
            for (const type of mediaTypes) {
                ...
                if (key === (subType||mainType) && message[mainType] === undefined) {
                    ...
                    else if (isReadableStream(value)) media[mainType] = { stream: value }
...
async function editMessage(message, text) {
    const socket = getSocket()
    if (!socket) return null
    if (!message) throw new Error("editMessage: No target message specified")
    if (typeof text !== "string" || !text) throw new Error("editMessage: newText must be a non-empty string")
    const room = message.room
    const edit = new Message(
        await queue("socket", null, () => socket.sendMessage(room, {
            edit: message.toBaileys().key,
            text
        }))
    )
    messageStore.add(edit.toBaileys())
    return edit
}
